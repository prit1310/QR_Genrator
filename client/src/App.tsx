...
function App() {
  ...
  const sendTotalAmount = async () => {
    try {
      const response = await fetch("http://localhost:5000/update-total", {
        method: "POST",
        headers: {
  ...
  const generateQrCodeUrl = async () => {
    try {
      const response = await fetch(`http://localhost:5000/qrcode?totalAmount=${totalAmount}`);
  ...
  return (
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col items-center p-6">
      <h1 className="text-5xl font-extrabold text-blue-700 mb-10">QR Genrator</h1>
